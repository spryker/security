...
class EntryPointPrototypesServiceLoader implements ServiceLoaderInterface
{
     ...
     * @var string
     */
    protected const AUTHENTICATOR_PLACEHOLDER = 'security.%s.%s.authenticator';
    ...
    protected function addEntryPointFormPrototype(ContainerInterface $container): ContainerInterface
    {
        $container->set(static::SERVICE_SECURITY_ENTRY_POINT_FORM_PROTO, $container->protect(function (string $firewallName, array $options) use ($container): callable {
            return static function () use ($container, $options, $firewallName): ?AuthenticatorInterface {
                ...
                foreach (static::ENTRY_POINTS as $entry_point) {
                    $authenticator = sprintf(static::AUTHENTICATOR_PLACEHOLDER, $firewallName, $entry_point);

                    if ($container->has($authenticator)) {
                        return $container->get($authenticator);
                    }
                }
